...
\
  ...
  if { [catch {set counter_1 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
    ...
    set_property CONFIG.data_width {64} $counter_1


  # Create instance: build_info_0, and set properties
  set block_name build_info
  set block_cell_name build_info_0
  if { [catch {set build_info_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $build_info_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  [get_bd_pins registers_0/build_ver]
  connect_bd_net -net counter_0_value  [get_bd_pins counter_0/value] \
